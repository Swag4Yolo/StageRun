...
def compute_upper_bound(programs: List[ProgHandler]) -> int:
    longest = max((len(p.queue) for p in programs), default=0)
    return max(0, longest - 1)

def consume_wp(programs: List[ProgHandler], state: PipeState,
...
def consume_instr(programs: List[ProgHandler], state: PipeState,
                  stages: List[Stage], stage_idx: int, table_idx: int):
    new_programs = deepcopy(programs)
    new_state = deepcopy(state)
    table = stages[stage_idx].tables[table_idx]
...
def next_table_or_stage(programs: List[ProgHandler], state: PipeState,
                        stages: List[Stage],
...
def pipeline_branch(handlers: List[ProgHandler], 
                    state: PipeState,
                    stages: List[Stage],
                    stage_idx: int, 
                    table_idx: int,
    ...
    if all(p.empty() for p in handlers):
        # no more searching
        return recircs, state
    # prone tree
    ...
    is_wp_available = any((not p.empty()) and p.head().kind == "WRITE_PHASE" for p in handlers)
    results = []
    ...
    if is_wp_available:
        # new wp
        if not is_stage_wp:
            progs1, state1 = consume_wp(handlers, state, stages, stage_idx, table_idx)
            r1 = next_table_or_stage(progs1, state1, stages, stage_idx, table_idx, recircs, upper_bound)
            results.append(r1)
            progs2, state2, progressed = consume_instr(handlers, state, stages, stage_idx, table_idx)
            if progressed:
                r2 = next_table_or_stage(progs2, state2, stages, stage_idx, table_idx, recircs, upper_bound)
            else:
                r2 = next_table_or_stage(handlers, state, stages, stage_idx, table_idx, recircs, upper_bound)
            results.append(r2)
        else:
            # already a wp, only consume instrs
            progs3, state3 = consume_wp(handlers, state, stages, stage_idx, table_idx)
            r3 = next_table_or_stage(progs3, state3, stages, stage_idx, table_idx, recircs, upper_bound)
            results.append(r3)
    else:
        if is_stage_wp:
            r = next_table_or_stage(handlers, state, stages, stage_idx, table_idx, recircs, upper_bound)
            results.append(r)
        else:
            progs4, state4, progressed = consume_instr(handlers, state, stages, stage_idx, table_idx)
            if progressed:
                r4 = next_table_or_stage(progs4, state4, stages, stage_idx, table_idx, recircs, upper_bound)
            else:
                r4 = next_table_or_stage(handlers, state, stages, stage_idx, table_idx, recircs, upper_bound)
            results.append(r4)
