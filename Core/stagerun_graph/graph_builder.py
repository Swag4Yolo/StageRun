...
class StageRunGraphBuilder:
    ...
    def _new_node(self, kind, instr, effect):
        nid = self._next_id
        self._next_id += 1
        node = StageRunNode(id=nid, kind=kind, instr=instr, effect=effect)
        self.nodes[nid] = node
        return node

    def _add_edge(self, src, dst, dep, label=None):
        self.edges.append((src, dst, dep, label))

    def _finalize(self) -> StageRunGraph:
        g = StageRunGraph(graph_id=self.graph_id)
        for node in self.nodes.values():
            g.add_node(node)
        for (src, dst, dep, label) in self.edges:
            g.add_edge(src, dst, dep, label)
        return g
    ...
    def build_from_instructions(self, pf_name: str, instructions: list):
        ...
        for instr in instructions:
            ...
            node = self._new_node("INSTR", instr, eff)

            # 1️⃣ ligar sequencialmente (fallthrough)
            for p in prev_nodes:
                self._add_edge(p, node.id, "FALLTHROUGH")

            # 2️⃣ dependências de dados
            for read in eff.reads:
                if read in self.last_writer:
                    self._add_edge(self.last_writer[read], node.id, "DATA")

            # 3️⃣ dependências de recursos (port, queue, pattern, etc.)
            for res in eff.uses:
