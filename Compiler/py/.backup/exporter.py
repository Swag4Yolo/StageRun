...
def _serialize_bool_expr(expr):
    if expr is None:
        return None
    if isinstance(expr.left, BooleanExpression):
        left = _serialize_bool_expr(expr.left)
    else:
        left = expr.left
    if isinstance(expr.right, BooleanExpression):
        right = _serialize_bool_expr(expr.right)
    else:
        right = expr.right
    return {"left": left, "op": expr.op, "right": right}
...
def _serialize_instr(instr):
    ...
    if isinstance(instr, FwdInstr):
        return {"op": "FWD", "args": {"dest": instr.port}}
    ...
    if isinstance(instr, CloneInstr):
        return {"op": "CLONE", "args": {"dest": instr.port}}
...
def _serialize_effects(n: StageRunNode):
    return {"effect": {
...
def _serialize_node(n: StageRunNode) -> Dict[str, Any]:
    ...
    payload.update(_serialize_effects(n))
