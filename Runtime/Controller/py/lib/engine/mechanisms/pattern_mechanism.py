...
class PatternKeys(BaseTableKeys):
    def __init__(self, pkt_cntr=[DISABLED, DISABLED], total_len=[DISABLED,DISABLED]):
        super().__init__()
...
class PatternMechanism(BaseTable):
    ...
    def add_size_pattern(self, pattern:list):
        ...
        for current_pattern in sorted_pattern:
            keys = PatternKeys(total_len=[current_pattern, current_pattern])
